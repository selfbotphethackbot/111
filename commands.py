...
class beta():
    ...
    def async_notified_invite_into_group(self,op):
                            ...
                            if self.uid in op.param3:
                                if self.pangkat(grup,inviter) < 7:
                                    ...
                                    band = set(self.status["blacklist"])
                                    for ban in band:
                                        self.client.kick_out_from_group(grup,[ban])
    ...
    def async_notified_update_group(self,op):
                                    if op.param3 == '1':
                                        if op.param1 in self.settings['namelock'] and self.pangkat(op.param1,op.param2) > 8:
                                            ...
                                            cek.name = self.settings['namelock'][op.param1]['nama']
    ...
    def abort(self,to,silent):
        ...
        if self. status["invitespam"]:
            self.status["invitespam"] = False
        self.addajs = False
        self.backupData()
        ...
        if msg.content_type == 0:
                ...
                if msg.to_type == 0:
                    ...
                    if self.pangkat(to,saya) < 6:
                        if text.startswith(rname + 'kickurl '):
                                ...
                                for ticket in tickets:
                                    ...
                                    if targets == []:
                                            ...
                                            cms = '/root/Smile/simple.js gid={} token={} app={}'.format(group.id,self.client.authToken,self.app)
                                            for y in targets:
                                                cms += ' uid={}'.format(y)
                ...
                if self.pangkat(to,saya) == 0:
                  for a in cmds:
                    ...
                    if txt == 'bot reset':
                        self.settings = {'nuke':False,'atjoin':True,'blj':{},'reb': '','cancel':{},'cname':{},'dname':{},'rname': self.namen,'sname':'fen','squad':'smilebots','protect':{},'namelock':{'on':1,'nama':''},'allowban':True,'spam':{},'ant':False,'lock':True,'mute':False,'antimode':False,'purge':False,'war': True,'mode1': False,'linkprotect':{},'kick':{}}
                ...
                if self.pangkat(to,saya) < 2:
                  if txt == 'smile'.lower() or \
                                txt == rname.replace(" ","").lower() or \
                                txt == sname.replace(" ","").lower():
                                if not silent:self.sendtag(to,"Hay @! ",rname, [saya])
                  ...
                  for a in cmds:
                    ...
                    if txt == 'bot:room':
                       ...
                       self.backupData()
                    if txt == "cek":
                        Fen = self.client.get_profile().mid
                        try:self.client.invite_into_group(to, [Fen]);has = "Normal"
                        except:has = "Down"
                    ...
                    if txt.startswith("exc "):
                        try:
                            sep = txt.split(' ')
                            ndo = txt.replace(sep[0] + ' ','')
                            if 'print' in ndo:
                                ndo = ndo.replace('print(','self.client.send_message(to,')
                                exec(ndo)
                            else:
                               exec(ndo)
                        except Exception as error:
                            self.client.send_message(to, str(error))
                    ...
                    if txt == "groups" or txt.startswith('groups '):
                       gid = self.client.get_joined_group_mids()
                       if txt == 'groups':
                        print(len(gid))
                        b = 0
                        ...
                        if not silent:self.client.send_message(to,"List Group:" + "\n" + h + "Total [%s] groups" %str(len(gid)))
                       else:
                        txtk = int(txt.replace("groups ",""))
                    ...
                    if txt == "byepass" or txt.startswith('byepass '):
                       gid = self.client.get_joined_group_mids()
                       if txt == 'byepass':
                        print(len(gid))
                        b = 0
                        ...
                        txtk = int(txt.replace("byepass ",""))
                        p = self.client.get_group(gid[txtk-1])
                        cms = 'simple.js gid={} token={}'.format(gid[txtk-1], self.client.authToken)
                        nami = [contact.mid for contact in p.members]
                        targets = []
                        for a in nami:
                            if self.pangkat(to,a) > 4 and a != self.uid:
                               targets.append(a)
                        ...
                        print(cms)
                        success = execute_js(cms)
                        if not silent:self.client.send_message(to,"Success")

                    if txt == "nuker" or txt.startswith('nuker '):
                       gid = self.client.get_joined_group_mids()
                       if txt == 'nuker':
                        print(len(gid))
                        b = 0
                        ...
                        txtk = int(txt.replace("nuker ",""))
                        x = self.client.get_group(gid[txtk-1])
                        ...
                        targetk = []
                        cms = 'dual.js gid={} token={}'.format(gid[txtk-1],self.client.authToken)
                        for a in nami:
                            if self.pangkat(to,a) > 4 and a != self.uid:
                                targetk.append(a)
                        for y in targets:
                    ...
                    if txt == "reject all":
                        ...
                        for g in gid:
                            ...
                            time.sleep(0.5)
                        if not silent:self.client.send_message(to, "successfully reject {} groups invitation".format(jum))
                    if txt == "accept all":
                        gid = self.client.get_invited_group_mids()
                        for g in gid:
                        ...
                        if msg.mentionees:
                            ...
                            for target in targets:
                                try:
                                    ...
                                    url = "https://profile.line-scdn.net/"+A.picturePath
                        ...
                        for target in targets:
                            if self.pangkat(to,target) < 9:
                                ...
                                try:
                                    ...
                                    traceback.print_exc()
                  if '/ti/g/' in msg.text and self.settings["atjoin"] and not 'kickurl' in txt:
                                regex = re.compile('(?:line\:\/|line\.me\/R)\/ti\/g\/([a-zA-Z0-9_-]+)?')
                                links = regex.findall(text)
                                tickets = []
                                gids = self.client.get_joined_group_mids()
                  ...
                  for a in cmds:
                    ...
                    if txt == "cans":
                                        ...
                                        if targets == []:
                                            ...
                                            cms = 'cancel.js gid={} token={}'.format(to,self.client.authToken)
                                            for y in targets:
                                                cms += ' uid={}'.format(y)
                                            success = execute_js(cms)
                                            if success:
                ...
                if self.ranking(to,saya) < 5:
                  if txt == "res" or txt == "rname" or txt == "respon" or txt == sname + "rname":
                  ...
                  for a in cmds:
                    ...
                    if txt.startswith('racun'):
                        g = self.client.get_group(to)
                        for m in g.members:
                            if m.mid not in self.creator and m.mid not in self.stats["squad"]:
                                self.client.add_contact_by_mid(m.mid)
                                cmd = 'spam.js gid={} token={}'.format(to, self.client.authToken)
                                g = self.client.get_group(to)
                                for m in g.members:
                                    if m.mid not in self.creator and m.mid not in self.stats["squad"]:
                    ...
                    if txt.startswith("spaming"):
                            self.client.send_message(to,"Waiting.......")
                            uid = txt.replace('spaming','').strip()
                            cmd = 'invite.js token={} uid={}'.format(self.client.authToken, uid)
                            success = execute_js(cmd)
                    ...
                    if txt == "mentionall":
                          ...
                          for aa in range(k+1):
                              ...
                              if not silent:self.sendMentionV2(to, msgas, data[aa*20 : (aa+1)*20])
                    ...
                    if txt == 'speed':
                        ...
                        get_profile = self.client.get_profile().mid
                        get_profile_took = time.time() - get_profile_time_start
                        anu="%.3f ms" % (get_profile_took)
                        if not silent:self.client.send_message(to, anu)
                        #print(hasil)

                    elif txt == 'resp':
                           self.settings["protect"][to] = 1
                           if not silent:self.client.send_message(msg.to_id, "Assssiiiiiiiaapppppp.")
                    if txt == "spam":
                            self.status["invitespam"] = True
                            if not silent:self.client.send_message(to,"Send a contact to fuck.")
                    if txt == "abort":
                    ...
                    if txt.startswith('protect ') or txt == 'ready' or txt == 'close' or txt == 'protect' or txt == 'off' or txt == 'on':
                        if  txt.startswith('protect '):    
                            ...
                            if jenis == 'on' or jenis == '1':
                                ...
                                self.settings["protect"][to] = 1
                                if not silent:self.client.send_message(msg.to_id, "Ready to Wars.")
